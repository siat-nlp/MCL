    ...
    def build_vocab(self, data):
        ...
        for name in data[0].keys():
            ...
            if isinstance(field, TextField):
                ...
                if name == 'kb':
                    xs += [list(map(lambda s: s.replace('_', ' '), x[name])) for x in data]
        ...
        with open(data_file, "r") as fr:
            for line in fr:
                ...
                for i, t in enumerate(range(0, len(uids), 2)):
                    if t == 0:
                        u_sent = dialogs[t]
                        u_sent_b = " ".join([self.ent_token if w in gold_entity[i] else w
                                           for w in dialogs[t].split(" ")])
                        u_sent_e = " ".join([w if w in gold_entity[i] else self.nen_token
                                           for w in dialogs[t].split(" ")])
                        s_sent = dialogs[t + 1]
                        ...
                        s_sent_e = " ".join([w if w in gold_entity[i] else self.nen_token
                                           for w in dialogs[t + 1].split(" ")])
                    src.append(u_sent)
                    src_e.append(u_sent_e)
                    src_b.append(u_sent_b)
                    tgt.append(s_sent)
        ...
        for raw_data in tqdm(data):
            ...
            for name, strings in raw_data.items():
                if name in self.fields.keys():
                    ...
                    example[name] = self.fields["src"].numericalize(strings)
                else:
