...
class DialogBatcher(object):
    ...
    def get_batch(self, batch_idx):
        ...
        return batch_data
    def get_unprepared_batch(self, batch_idx):
        local_data = self.batch_data_list[batch_idx]
        return local_data
        ...
        for t in range(max_turn):
            ...
            for sample in sorted_data:
                if sample['turn'] >= t+1:
                    ...
                    turn_src_b.append(sample['src_b'][t])
                    turn_src_e.append(sample['src_e'][t])
                    turn_tgt.append(sample['tgt'][t])
                    turn_tgt_e.append(sample['tgt_e'][t])
                    turn_tgt_b.append(sample['tgt_b'][t])
            ...
            assert len(turn_tgt_b) == turn_batch_size
            assert len(turn_tgt_e) == turn_batch_size
...
def create_turn_batch(data_list):
    ...
    for data_dict in data_list:
        ...
        for key in data_dict.keys():
            if key in ['src', 'src_b', 'src_e', 'tgt', 'tgt_e', 'tgt_b', 'ptr_index', 'kb_index']:
