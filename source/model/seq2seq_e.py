...
from source.module.rnn_decoder_e import RNNDecoder
...
class Seq2Seq_E(BaseModel):
    ...
    def __init__(self,
        ...
        # loss definition
        if self.padding_idx is not None and self.nen_idx is not None:
            weight = torch.ones(self.tgt_vocab_size)
            weight[self.padding_idx] = 0
            weight[self.nen_idx] = self.nen_weight
        else:
            weight = None
    ...
    def forward(self, enc_inputs, dec_inputs, hidden=None):
        ...
        outputs.add(logits=log_logits, gate_logits=gate_logits,
                    ...
                    prob=logits)
        return outputs
    def collect_mpl_metric(self,inputs, target):
        loss = 0

        # loss for generation
        loss = self.kd_loss(inputs, target)

        return loss
    def collect_metrics(self, outputs, target,kb_index):
        """
        collect_metrics
        """
        ...
        loss_ptr = torch.tensor(0.0).cuda()
        loss += loss_ptr

        # acc = accuracy(logits, target, padding_idx=self.padding_idx)
        metrics.add(loss=loss,logits=logits, prob=outputs.prob)
    ...
    def iterate(self, turn_inputs, kb_inputs,
        ...
        for i, inputs in enumerate(turn_inputs):
            ...
            if use_rl:
                ...
                outputs = self.forward(enc_inputs, dec_inputs)
                metrics = self.collect_metrics(outputs, target,kb_index)
        ...
        return metrics_list,total_loss
