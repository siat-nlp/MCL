...
class NLLLoss(_Loss):
    ...
    def forward(self, input, target, mask=None, reduction=True, matrix=True):
        """
        input: (batch_size, max_len, vocab_size)
        target: (batch_size, max_len)
        """
        batch_size = input.size(0)
        input1 = input.view(-1, input.size(-1))
        target1 = target.contiguous().view(-1)
...
class KDLoss(_Loss):
    ...
    def forward(self, input, target, mask=None):
        """
        input: (batch_size, max_len, vocab_size) logit_S
        target: (batch_size, max_len, vocab_size)prob_TB
        mask: (batch_size, max_len) kd_mask_b
        """
        assert input.size() == target.size()
        kd_loss = input * target
